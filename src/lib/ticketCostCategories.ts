...
export const normalizeTicketCostCategory = ({ categoria, tipo, concepto }: TicketCostCategoryInput) => {
    ...
    // Priorizar detección por concepto cuando contiene keywords claras (p.ej. 'adelanto operativo')
    if (text.includes('adelanto operativo') || text.includes('adelanto m.o') || text.includes('adelanto mo')) return 'Adelanto Operativo';
    if (text.includes('adelanto') && !text.includes('liquidación') && !text.includes('liquidacion')) return 'Adelanto Operativo';
    ...
    if (text.includes('liquidación') || text.includes('liquidacion') || text.includes('saldo de mano de obra') || text.includes('saldo mo')) return 'Mano de Obra';
