...
export async function POST(req: NextRequest) {
    ...
    const diagLog: any = {
        ...
        webhook: 'power-automate-tickets',
    ...
    const log = (step: string, detail: any) => {
        console.log(`[power-automate] ${step}:`, detail);
    ...
    try {
        ...
        if (!match) {
            ...
            const fallback = {
                ...
                created_by: 'SISTEMA (PA-WEBHOOK FAIL)',
        ...
        const { data: existing } = await supabase
            .from('tickets').select('id').eq('client_ticket_number', ticket_banco).eq('client_id', MIBANCO_ID).maybeSingle();

        if (existing) {
            log('DUPLICADO', { id: existing.id });
            return NextResponse.json({ id: existing.id, message: `Duplicado: ${ticket_banco}`, duplicate: true }, { status: 200 });
        }

        let finalBranchId: string | null = null;

        const { data: byCode } = await supabase
            .from('branch_offices').select('id, name').eq('client_id', MIBANCO_ID)
            .ilike('codigo_cliente', `%${codigo_sede}%`).maybeSingle();
        if (byCode?.id) { finalBranchId = byCode.id; log('SEDE_CODIGO', byCode.name); }

        if (!finalBranchId && cleanNombre.length >= 4) {
            const { data: byName } = await supabase
                .from('branch_offices').select('id, name').eq('client_id', MIBANCO_ID)
                .ilike('name', `%${cleanNombre}%`).maybeSingle();
            if (byName?.id) { finalBranchId = byName.id; log('SEDE_NOMBRE', byName.name); }
        }

        if (!finalBranchId) {
            const { data: all } = await supabase.from('branch_offices')
                .select('id, name, codigo_cliente').eq('client_id', MIBANCO_ID);
            const fuzzy = all?.find(b => {
                const dl = b.name.toLowerCase();
                const rl = inmueble_raw.toLowerCase();
                const cl = cleanNombre.toLowerCase();
                const cdl = (b.codigo_cliente || '').toLowerCase();
                return rl.includes(dl) || dl.includes(cl) || (cdl && rl.includes(cdl));
            });
            if (fuzzy) { finalBranchId = fuzzy.id; log('SEDE_FUZZY', fuzzy.name); }
        ...
        const ticketToCreate: any = {
            client_id: MIBANCO_ID,
            status_id: 'borrador',
            description: body || `Importado vía Power Automate. Tipo: ${tipo_incidencia}\nAsunto: ${subject}`,
            client_ticket_number: ticket_banco,
            service_type: null,
            created_by: 'SISTEMA (POWER-AUTOMATE)',
            ...
            metadata: {
                ...
                sede_reportada_cliente: inmueble_raw, tipo_incidencia_reportada: tipo_incidencia,
                origen: 'CORREO_A_TICKET', enrutamiento_cascada_aplicado: true,
                gestora_auto_asignada: !!gestoraId, sede_mapeada: !!finalBranchId
            }
        };
        ...
        console.error('[power-automate] EXCEPCION:', error);
