...
export default function TicketsPage() {
    // Usar contexto global de datos (Realtime compartido entre m├│dulos)
    const { tickets, loadingTickets, createTicket, updateTicket, refreshTickets } = useAppData();
    const queryClient = useQueryClient();
    const [showWizard, setShowWizard] = useState(false);
    const [openTickets, setOpenTickets] = useState<any[]>([]);
    
    // ­ƒÄ½ OFFLINE-FIRST: Cargar del cache primero para scroll instant├íneo
    useEffect(() => {
    ...
    // ÔöÇÔöÇ GESTORA RESOLUTION Y ROLES ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    ...
    // ­ƒÜÇ Cache inicial de identidad en sessionStorage para evitar flash del empty state
    ...
    const [isAdminState, setIsAdminState] = useState<boolean>(() => {
        ...
        // escribe cookies ÔÇö sin este fallback el admin queda con isAdminState=false
        // y la bandeja sale vac├¡a).
    ...
    const fetchGestora = useCallback(async () => {
        try {
            // ­ƒöº Helper: leer una cookie por nombre (Azure AD callback escribe en cookies)
            ...
            // REVISI├ôN DE ROL EN PERFILES (Independiente de si es gestora o no)
            ...
            // Persistir en sessionStorage para que la pr├│xima navegaci├│n sea instant├ínea
    ...
    const isVisibleForMe = useCallback((t: any) => {
        ...
        // CASCADA (Solo si no hay gestora directa): ┬┐A qui├®n le toca tomarlo?
        // 1. Asignaci├│n por Sede (Branch)
        if (t.branch_offices?.gestora_asignada_id === myGestoraId) return true;

        // 2. Asignaci├│n por Zona (Cascada 1)
        if (t.branch_offices?.zonas?.gestora_asignada_id === myGestoraId) return true;

        // 3. Asignaci├│n por Cliente o Sede-Cliente (Cascada 2)
    ...
    const handleCreateTicket = async (newTicket: any) => {
        // ­ƒÜÇ createTicket ya inserta en cache local (setQueryData en AppDataContext)
        // y el canal realtime mantiene sincron├¡a. Eliminamos refreshTickets() redundante
        // que disparaba un refetch HTTP completo y bloqueaba el cierre del modal.
        await createTicket(newTicket);
    };

    // ­ƒÜÇ L├ôgica de Limpieza de Tickets (One-time)
    useEffect(() => {
        ...
        if (shouldCleanup === 'tickets') {
            ...
            alert("Los tickets han sido eliminados del sistema. El sistema se reiniciar├í.");
    ...
    const handleOpenTicket = (ticket: any) => {
        if (typeof window !== 'undefined') {
            // ­ƒÜÇ REGLA DE ORO: Si el usuario hace clic en la tarjeta, quiere VER el ticket.
        ...
        if (!openTickets.find(t => t.id === ticket.id)) {
            ...
            // Si ya est├í abierta, avisar a la ventana que se maximice/traiga al frente
            window.dispatchEvent(new CustomEvent('ticket-reopen', { detail: { ticketId: ticket.id } }));
        }
    };

    // ­ƒøá´©Å REPARACI├ôN/SINCRONIZACI├ôN AUTOM├üTICA
    // Eliminada la sincronizaci├│n local forzada, ahora se usa normalizaci├│n en el hook useTickets
    ...
    // ÔöÇÔöÇ B├ÜSQUEDA GLOBAL: helpers de filtrado ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    ...
    // ÔöÇÔöÇ B├ÜSQUEDA GLOBAL (all states) vs vista filtrada ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    ...
    const filteredTickets = useMemo(() => {
        // B├ÜSQUEDA GLOBAL: Cuando hay texto, se ignora el viewMode y se muestran TODOS
    ...
    return (
        <div className={styles.page}>
                        ...
                        <p className={styles.pageSubtitle}>
                            {viewMode === "triage" ? `${stats.borradores} por clasificar` :
                                ...
                                viewMode === "active" ? `${stats.activosTotal} activos` :
                                    `${stats.completados} cerrados`} ÔÇó {stats.nuevos} nuevos
                        </p>
                    </div>
                </div>
            ...
            <div className={styles.statsGrid}>
                ...
                {isAdminState && (
                    <div 
                        ...
                        title="Ver tickets que requieren aprobaci├│n"
                    >
            ...
            {/* ­ƒöì TOOLBAR CON B├ÜSQUEDA GLOBAL */}
            <div className={styles.toolbar}>
                <div className={styles.searchBox} style={{
                    ...
                    <input
                        type="text"
                        placeholder="­ƒöì Buscar en TODOS los estados: ID, Cliente, Sede, T├®cnico, Gestor..."
                        value={searchTerm}
            ...
            {/* ­ƒôó BANNER DE B├ÜSQUEDA GLOBAL */}
                ...
                <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        <span style={{ fontSize: '0.82rem', fontWeight: 700, color: '#4338CA' }}>
                            B├║squeda global: &ldquo;{searchTerm}&rdquo;
                        </span>
                    </div>
            ...
            {/* ­ƒÄ½ LISTA CON SEPARADORES POR SECCI├ôN EN MODO B├ÜSQUEDA GLOBAL */}
            <div className={styles.ticketsList}>
                {(() => {
                    // ­ƒÜÇ Mientras carga (tickets o identidad del gestor) NO mostrar empty state
                ...
                {filteredTickets.length === 0 && !loadingTickets && (gestoraResolved || isAdminState || myGestoraId) ? (
                    <div className={styles.emptyState}>
                        ...
                        <h3>{isSearching ? `Sin resultados para "${searchTerm}"` : (viewMode === 'active' ? '┬íComienza tu d├¡a productivo!' : 'No hay historial acumulado')}</h3>
                        <p>{isSearching ? 'Intenta con otro t├®rmino: n├║mero de ticket, cliente, sede, t├®cnico o gestor.' : (viewMode === 'active' ? 'Crea tu primer ticket y gestiona tus servicios' : 'Los tickets aparecer├ín aqu├¡ una vez que sean liquidados y cerrados.')}</p>
                    ...
                    // ÔöÇÔöÇ MODO B├ÜSQUEDA GLOBAL: Mostrar todos agrupados por secci├│n ÔöÇÔöÇÔöÇ
                    <div>
                        {(['triage', 'active', 'closed'] as const).map((section) => {
                            const sectionLabel = section === 'triage' ? '­ƒôï Triage' : section === 'active' ? 'ÔÜí En Proceso' : 'Ô£à Cerrados';
                            ...
                            return (
                                <div key={section} style={{ marginBottom: '1.5rem' }}>
                                    <div style={{
                                        ...
                                        <span style={{ fontSize: '0.75rem', color: '#94A3B8', fontWeight: 600 }}>ÔÇö {sectionTickets.length} ticket{sectionTickets.length !== 1 ? 's' : ''}</span>
                                    </div>
            ...
            {/* ­ƒ¬ƒ VENTANAS FLOTANTES DE TICKETS */}
            {
                openTickets.map((ticket, index) => {
                    // ­ƒöÑ FRESH DATA: Buscar la versi├│n m├ís reciente del ticket en el estado global (Realtime)
                    // Esto asegura que si el Admin confirma un pago, la Gestora lo vea reflejado al instante
                    // sin tener que cerrar y abrir el ticket, ya que 'tickets' se actualiza v├¡a suscripci├│n.
...
function TicketCard({ ticket, onTicketClick, isDraggable, provided }: any) {
    ...
    return (
        ...
        >
            ...
            {/* Cabecera GEOM├ëTRICA (El n├║mero es el Centro) */}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                         ...
                         <span style={{ fontSize: '0.75rem', color: '#475569', fontWeight: 500 }}>
                            {ticket.sede?.direccion || 'Direcci├│n no especificada'}
                         </span>
                    </div>
...
function TicketRow({ ticket, onTicketClick }: any) {
    ...
    const totalCostos = pagos.reduce((acc: number, p: any) => acc + (parseFloat(p.monto) || 0), 0)
        + (ticket._ticketCostsSum || 0); // suma de ticket_costs si est├í disponible
        ...
        <tr className={styles.historyRow} onClick={onTicketClick}>
            ...
            <td>
                <div className={styles.rowLiquidationCol}>
                    ...
                    {/* COSTO REAL PAGADO ÔÇö con alerta si hay p├®rdida */}
                    {costoDisplay > 0 && (
                        <span style={{
                            fontSize: '0.68rem',
                            fontWeight: 700,
            ...
            <td>
                {ticket.gestora?.name || ticket.gestora?.nombre ? (
                    ...
                    <span className={styles.rowGestora} style={{ color: '#F59E0B', fontWeight: 800 }}>ÔÜá´©Å PENDIENTE</span>
                )}
            </td>
            <td>
