...
async function main() {
  ...
  const ticketsWithCosts = await attachTicketCosts(myTickets);

  console.log(`Analyzing ${ticketsWithCosts.length} tickets for Janeth:`);

  const now = new Date();

  // dateFilter = "week"
  const weekTickets = ticketsWithCosts.filter(t => {
    const d = new Date(t.createdAt || t.created_at || now.toISOString());
    const diff = (now.getTime() - d.getTime()) / 86_400_000;
    return diff < 7; // within 7 days
  });

  console.log(`\n--- Tickets within 7 days (dateFilter = "week"): ${weekTickets.length} ---`);
  ...
  weekTickets.forEach((t) => {
    ...
    const isApproved = APPROVED_EXEC_STATES.includes(state);
    ...
    State: ${state} (Approved: ${isApproved})
    materials_cost: ${t.materials_cost}, labor_cost: ${t.labor_cost}
    totalOpConfirmed: ${finances.totalOpConfirmed}
    inversion calculation: ${ticketInversion}
